...
function addGraph() {
  ...
  for (let i = 0; i < nodeArray.length; i++) {
    if (nodeArray[i].flag == 1) {
      updateLabel(
        nodeArray[i].id,
        nodeArray[i].minmaxValue,
        nodeArray[i].leftchild,
        nodeArray[i].rightchild
      );
    }
  }
}
...
network.on("click", function (params) {
  ...
  if (params.nodes.length == 1) {
    if (
      params.nodes[0] <=
      2 ** document.getElementById("depth-label").value - 1
    ) {
      let value = prompt("Enter the node value");
      if (value != null) {
        if (isNaN(value)) {
          ...
          updateLabel(
            params.nodes[0],
            value,
            nodeArray[params.nodes[0] - 1].leftchild,
            nodeArray[params.nodes[0] - 1].rightchild
          );
...
function copyValue() {
  console.log(nodeArray, values, "nodeArray");
  const filteredvalues = values.filter((subArray) => subArray.length !== 0);
  for (let i = 0; i < filteredvalues.length; i++) {
    nodeArray[i].nodeValue = filteredvalues[i][1];
  }
  pushValue();
}
...
function minmax() {
  ...
  for (let i = count; i > 0; i--) {
    ...
    if (
      nodeArray[i - 1].leftchild == null &&
      nodeArray[i - 1].rightchild == null
    ) {
      //update node value
      ...
      updateEdgeLabel(
        i,
        nodeArray[i - 1].nodeValue,
        nodeArray[i - 1].leftchild,
        nodeArray[i - 1].rightchild
      );
      //update node label
      updateLabel(
        i,
      ...
      nodes.update({
        id: i,
        label: nodeArray[i - 1].nodeValue,
        font: { color: "#000001", size: 25 },
      });
    }
    ...
    //if node is not a leaf node
      ...
      if (isMax) {
        ...
        if (nodeArray[i - 1].leftchild != null) {
          ...
          nodeArray[nodeArray[i - 1].leftchild - 1].nodeValue = parseInt(
            nodeArray[nodeArray[i - 1].leftchild - 1].nodeValue
          );
        ...
        if (nodeArray[i - 1].rightchild != null) {
          ...
          nodeArray[nodeArray[i - 1].rightchild - 1].nodeValue = parseInt(
            nodeArray[nodeArray[i - 1].rightchild - 1].nodeValue
          );
  ...
  var msg =
    "<span class ='light' >The optimal value is  " + nodeArray[0].nodeValue;
  //showoperation
  showOperations(msg);
  ...
  for (let i = 0; i < nodeArray.length; i++) {
    ...
    if (nodeArray[i].type == "max") {
      ...
      if (nodeArray[i].leftchild != null) {
        //convert left child value to integer
        nodeArray[nodeArray[i].leftchild - 1].nodeValue = parseInt(
          nodeArray[nodeArray[i].leftchild - 1].nodeValue
        );
      ...
      if (nodeArray[i].rightchild != null) {
        //convert right child value to integer
        nodeArray[nodeArray[i].rightchild - 1].nodeValue = parseInt(
          nodeArray[nodeArray[i].rightchild - 1].nodeValue
        );
